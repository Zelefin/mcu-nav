...
кодовий агент (Claude Code). Скіли викликаються як слеш-команди, наприклад
`/grill-me` чи `/implement`, і допомагають загострювати дизайн, тримати
архітектуру чистою та перетворювати ідеї на задачі й PR.
...
3. [Встановлення скілів mattpocock/skills](#3-встановлення-скілів-mattpocockskills)
4. [Збірка та тести](#4-збірка-та-тести)
5. [Командний робочий процес](#5-командний-робочий-процес)
6. [Шпаргалка по gh CLI](#6-шпаргалка-по-gh-cli)
7. [Правила, яких ми дотримуємось](#7-правила-яких-ми-дотримуємось)
...
### 1.2. Node.js (потрібен для CLI `skills`)

Скіли встановлюються через `npx`, тому потрібен Node.js (LTS, 20+).
Рекомендований спосіб без `sudo` — через [nvm](https://github.com/nvm-sh/nvm):
...
### 1.4. Claude Code (агент, який запускає скіли)

Скіли цього репозиторію націлені на агента **Claude Code**. Встановіть його
глобально через npm:
...
Запустити агента в корені проєкту:
...
```bash
git clone https://github.com/Zelefin/mcu-nav.git
cd mcu-nav
```

У репозиторії вже закомічений файл [`skills-lock.json`](skills-lock.json) — це
«lock-файл» скілів (аналог `package-lock.json`). Він фіксує, які саме скіли,
з якого джерела (`mattpocock/skills`) та з яким хешем очікує проєкт. Завдяки
цьому вся команда отримує **однаковий** набір скілів.

---

## 3. Встановлення скілів mattpocock/skills

### 3.1. Відновлення з lock-файлу (рекомендовано для команди)
...
їх читає Claude Code. Перевірити встановлене:

```bash
npx skills list
```

### 3.2. Встановлення «з нуля» (якщо lock-файлу ще немає)

Якщо ви ініціюєте набір скілів у новому репозиторії, додайте їх інтерактивно й
націльте на Claude Code — команда сама створить `skills-lock.json`:

```bash
npx skills add mattpocock/skills --agent claude-code
```

Інсталятор запропонує вибрати, які саме скіли поставити. Для нашого процесу
ключові:

| Скіл | Призначення |
| --- | --- |
| `setup-matt-pocock-skills` | Одноразове налаштування репозиторію під скіли |
| `grill-me` / `grilling` | Жорстке інтервʼю, щоб загострити план чи дизайн |
| `grill-with-docs` | Те саме + автоматично пише ADR та глосарій (`CONTEXT.md`) |
| `improve-codebase-architecture` | Пошук «поглиблень» архітектури + HTML-звіт |
| `codebase-design` / `domain-modeling` | Спільний словник для дизайну й домену |
| `to-prd` | Перетворити обговорення на PRD у трекері задач |
| `to-issues` | Розбити план на незалежні issue (вертикальні зрізи) |
| `implement` | Реалізувати задачу через TDD + `/review` |
| `triage` | Сортування вхідних issue за станами |

### 3.3. Оновлення та перевірка скілів
...
## 5. Командний робочий процес

Ми ведемо фічу по конвеєру скілів — від сирої ідеї до влитого PR. Кожен крок
виконується слеш-командою в Claude Code, а трекером задач слугують GitHub Issues.
...
### 5.1. Загострення ідеї: `/grill-me` та `/grill-with-docs`
...
### 5.2. Покращення архітектури: `/improve-codebase-architecture`
...
### 5.3. PRD та issue: `/to-prd` і `/to-issues`
...
## 7. Правила, яких ми дотримуємось
